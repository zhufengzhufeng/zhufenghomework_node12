    ...
    data.forEach(function (item, index) {
        str += `<li class="list-group-item">
           ID:${item.id}  用户名：${item.username}  密码：${item.password}
            ...
            <button type="button" class="bth btn-warning pull-right btn-xs" style="margin-right: 10px"    data-name="${item.username}" data-password="${item.password}" onclick="upDate(${item.id},this)" >修改</button>
        </li>`;
    });
    oul.innerHTML = str;
}
...
function upDate(id,that) {
    ...
    password.value = that.getAttribute('data-password');
